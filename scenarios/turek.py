...
class Turek(Scenario[TRTLBM]):
	def define(self, lbm: TRTLBM) -> None:
		...
		for y_idx in range(1, lbm.y - 1):
			y_p = y_phys[y_idx]
			u_in_si = lbm.us.magnitude(u_max * (4.0 * y_p * (lbm.height - y_p) / lbm.height**2))
			lbm.boundary.velocity_profile[y_idx, 0] = lambda step, s=u_in_si, t=t_ramp_si: s * (1 - math.exp(-((step / t) ** 2))) * np.array([1, 0])
...
if __name__ == "__main__":
	characteristic_quantities: list[ScalarQuantityDefinition] = [(1, "cm"), (3, "ms"), (1, "kg/m**3")]
