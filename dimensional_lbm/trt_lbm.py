...
def tau_plus_from_viscosity(viscosity: ScalarT, lattice: DdQqLattice[ScalarT, VectorT]) -> ScalarT:
	"""Compute the symmetric TRT relaxation time (tau_plus) from the kinematic viscosity."""
...
def tau_minus_from_magic_param(tau_plus: ScalarT, lattice: DdQqLattice[ScalarT, VectorT], magic: float = 3 / 16) -> ScalarT:
	"""Compute tau_minus from the magic parameter Λ = (τ+/dt - ½)(τ-/dt - ½)."""
	# Pint's scalar operator stubs widen the result type, so restore the known ScalarT.
	return cast("ScalarT", lattice.dt / 2 + magic * lattice.dt**2 / (tau_plus - lattice.dt / 2))


def _compute_opposite_lattice_indices(lattice: DdQqLattice[ScalarT, VectorT]) -> np.ndarray:
	num_dirs = lattice.Q
	dir_x = lattice.dir_x
	dir_y = lattice.dir_y
	opposite = np.zeros(num_dirs, dtype=int)
...
class TRTLBM(LBM[ModeT, ScalarT, VectorT]):
	...
	def collide(self) -> None:
		...
		f_minus = 0.5 * cast("VectorT", (self.f - self.f[self._op_ind]))
		feq_plus = 0.5 * cast("VectorT", (self.feq + self.feq[self._op_ind]))
		feq_minus = 0.5 * cast("VectorT", (self.feq - self.feq[self._op_ind]))
